...
export default async function Author({ blok, uuid }) {
	...
	return (
		<article
			...
			className="mx-auto max-w-3xl px-6 py-16 text-zinc-900"
		>
			<nav className="mb-12">
				<Link
					href="/authors"
				...
				>
					<span aria-hidden="true">←</span> Tillbaka till författare
				</Link>
			</nav>

			<section className="flex flex-col-reverse items-start justify-between gap-8 border-b border-zinc-200/80 pb-14 sm:flex-row sm:items-center ">
				...
				{blok.photo?.filename && (
					<img
						...
						width={200}
						height={200}
						className="size-50 shrink-0 rounded-full object-cover ring-1 ring-zinc-100/20 sm:size-40 "
					/>
				)}
			</section>
					...
					<ul className="divide-y divide-zinc-100 ">
						{posts.map((post) => (
							<li key={post.uuid}>
								...
								>
									<span className="text-base font-medium text-zinc-800 transition-colors group-hover:text-zinc-950">
										{post.content.title}
									</span>
									<span
										aria-hidden="true"
										className="text-sm text-zinc-800 transition-transform duration-150 group-hover:translate-x-1 group-hover:text-zinc-700"
									>
										→
									</span>
								</Link>
